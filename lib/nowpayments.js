...
export async function createPayment({ 
  ...
  const response = await fetch(`${API_BASE}/invoice`, {
    ...
    body: JSON.stringify({
      ...
      // Prefer explicit callback URL (over dashboard default) so each environment works reliably.
      ...(process.env.NEXT_PUBLIC_BASE_URL
        ? { ipn_callback_url: `${process.env.NEXT_PUBLIC_BASE_URL}/api/webhook` }
        : {}),
    }),
  })
  ...
  if (!response.ok) {
    ...
    const msg = `NOWPayments invoice error (${response.status}): ${baseMsg}`
