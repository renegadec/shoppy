...
import { HOT_ZESA_PRODUCT_ID } from '@/lib/currencies'
...
export async function fulfillZesaOrderIfPaid({ orderNumber }) {
  ...
  // If the customer paid in USD but ZESA is ZiG-only on Hot, convert.
  let tokenAmountForHot = order.tokenAmount
  let tokenCurrency = order.currency || 'USD'
  if (order.currency === 'USD') {
    const rate = await getPricingSettingFloat('usd_zig_rate', 25)
    tokenAmountForHot = order.tokenAmount * rate
    tokenCurrency = 'ZWG'
  }

  const productId = order.hotProductId || HOT_ZESA_PRODUCT_ID
