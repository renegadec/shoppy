...
function msisdnVariants(msisdn) {
  const normalized = normalizeEcoCashMsisdn(msisdn)
  const digits = String(msisdn || '').replace(/\D/g, '')
  const withoutCountryCode = normalized.startsWith('263') ? normalized.slice(3) : ''
  const local = withoutCountryCode ? `0${withoutCountryCode}` : ''
  return uniqueTruthy([normalized, withoutCountryCode, local, digits])
}
...
export async function getEcoCashC2BTransactionStatus({
  ...
  const endUserIdCandidates = uniqueTruthy([
    ...
    ...msisdnVariants(process.env.ECOCASH_MERCHANT_NUMBER),
    ...msisdnVariants(sourceMobileNumber),
  ])
