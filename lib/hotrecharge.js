...
/**
 * Query available stock/bundles for a product.
 * GET /api/v3/query/stock/{productId}
 */
export async function hotQueryStock({ productId }) {
  if (!productId) throw new Error('productId is required')

  const token = await getHotBearerToken()
  const url = `${getBaseUrl()}/api/v3/query/stock/${productId}`
  ...
  if (!res.ok) {
    const msg =
      ...
      data?.errors?.[0] ||
      `Hot stock query failed (${res.status})`
    const err = new Error(msg)
    err.status = res.status
    err.details = { hot: data }
    throw err
  }
...
/**
 * Query customer/account details for a product.
 * GET /api/v3/query/account/{productId}/{accountNumber}
 */
export async function hotQueryAccount({ productId, accountNumber }) {
  if (!productId) throw new Error('productId is required')
  if (!accountNumber) throw new Error('accountNumber is required')

  const token = await getHotBearerToken()
  const url = `${getBaseUrl()}/api/v3/query/account/${productId}/${encodeURIComponent(accountNumber)}`
  ...
  if (!res.ok) {
    const msg =
      ...
      data?.errors?.[0]?.message ||
      data?.errors?.[0] ||
      `Hot account query failed (${res.status})`
    const err = new Error(msg)
    err.status = res.status
    err.details = { hot: data }
    throw err
...
export async function hotRechargeProduct({
  ...
  const payload = {
    ...
    // Ensure string+number variants are both present.
    productId,
  ...
  console.log('Hot recharge request payload keys:', Object.keys(payload))
  ...
  if (!res.ok) {
    ...
    const err = new Error(msg)
    err.status = res.status
    err.details = { hot: data, sent: payload }
    throw err
  }
