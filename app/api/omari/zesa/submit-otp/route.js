...
export async function POST(request) {
  try {
    ...
    await prisma.zesaOrder.update({ where: { id: order.id }, data: { paymentStatus: 'omari_otp_submitted', providerRef: payment?.paymentReference || order.providerRef, deliveryNotes: JSON.stringify({ omariPayment: payment }) } })
    const status = await getOmariPaymentStatus(order.paymentId)
    if (String(status?.status || '').toUpperCase() === 'SUCCESS') {
      await prisma.zesaOrder.update({ where: { id: order.id }, data: { status: 'PAID', paymentStatus: 'omari_success', paidAt: new Date(), paidAmount: status?.amount ?? order.amount, paidCurrency: status?.currency ?? order.currency, providerRef: status?.paymentReference || payment?.paymentReference || order.providerRef } })
