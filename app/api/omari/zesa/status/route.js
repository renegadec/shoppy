...
export async function POST(request) {
  try {
    ...
    if (s === 'SUCCESS') {
      await prisma.zesaOrder.update({ where: { id: order.id }, data: { status: 'PAID', paymentStatus: 'omari_success', paidAt: new Date(), paidAmount: status?.amount ?? order.amount, paidCurrency: status?.currency ?? order.currency, providerRef: status?.paymentReference || order.providerRef } })
      await fulfillZesaOrderIfPaid({ orderNumber })
    } else {
      await prisma.zesaOrder.update({ where: { id: order.id }, data: { paymentStatus: `omari_${String(status?.status || 'pending').toLowerCase()}`, providerRef: status?.paymentReference || order.providerRef } })
    }
